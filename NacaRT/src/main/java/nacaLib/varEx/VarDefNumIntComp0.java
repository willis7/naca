...
//	}
	...
	{
		return buffer.getAsUnsignedInt(buffer.m_nAbsolutePosition, m_nTotalSize);
	}
...
//	}
	...
	{
		if(!m_bBlankWhenZero)
			RWNumIntComp0.setFromRightToLeft(buffer, 0, nValue, m_nTotalSize, m_nNbDigitInteger);
		else
		{
			if(nValue != 0)
				RWNumIntComp0.setFromRightToLeft(buffer, 0, nValue, m_nTotalSize, m_nNbDigitInteger);
...
//		Pic9Comp0BufferSupport pic9Comp0BufferSupport = new Pic9Comp0BufferSupport(buffer, buffer.m_nAbsolutePosition, m_nTotalSize, m_nTotalSize);
//		pic9Comp0BufferSupport.setFromRightToLeft(0, nValue);
	}
	
	void writeIntComp0(VarBufferPos buffer, int nOffset, int nValue)
	{
		//RWNumIntComp0.internalWriteAbsoluteIntComp0(buffer, nOffset, nValue, buffer.m_nAbsolutePosition, m_nTotalSize);
		if(!m_bBlankWhenZero)
			RWNumIntComp0.setFromRightToLeft(buffer, nOffset, nValue, m_nTotalSize, m_nNbDigitInteger);
		else
		{
			if(nValue != 0)
				RWNumIntComp0.setFromRightToLeft(buffer, nOffset, nValue, m_nTotalSize, m_nNbDigitInteger);
...
//		String cs2 = getStringRightPadded(n2, ' ', varDef1.getTotalSize());
	...
	private boolean m_bBlankWhenZero;
}
