    ...
    synchronized void LoadConfig(ControlerDirector director) {
        ...
        if (tagRoot != null) {
            ...
            while (curgrp.isValid() && tagGroup != null) {
                ...
                if (grpConfig == null) { // if task doesn't exist yet
                ...
                while (cur.isValid() && tagSite != null) {
                    ...
                    if (stepConf == null) { // step doesn't exist yet
                        bNewStep = true;
                    ...
                    if (bNewStep && !bNewTask) { // the step is new, but not the task : tel the director
                        director.AddStepToTask(grpConfig/*, nStepIndex, stepConf*/);
                    } else if (!bNewStep) { // remove step of known steps
                ...
                if (!tabSteps.isEmpty()) { // known steps have not been found in XML : remove them
...
//						int nIndex = grpConfig.FindStepConfig(conf);
                ...
                if (bNewTask) { // the task is new : create controler
                    director.AddNewTask(grpConfig);
                } else { // remove task from known tasks
            ...
            while (cur.isValid() && tagSite != null) {
                ...
                BaseControlerTaskConfig grpConfig = tabTasks.get(name);
                boolean bNewTask = false;
                if (grpConfig == null) { // the task is new
                    bNewTask = true;
                    grpConfig = NewTaskConfig(name);
                } else if (grpConfig.getNbSteps() > 1) { // the task exists but with more than one step : delete and remake
//					grpConfig.OnDeleteConfig() ;
                ...
                if (grpConfig.getNbSteps() > 0) { // single step already exists
                ...
                if (bNewTask) { // task is new : create a controler
                    director.AddNewTask(grpConfig);
                } else { // reove task from known tasks
                    tabTasks.remove(name);
                }
                tagSite = tagRoot.getNextChild(cur);
            }
